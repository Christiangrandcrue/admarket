-- AdMarket Database Schema for Supabase
...
CREATE TABLE public.campaigns (
  ...
  name TEXT NOT NULL,
  goal TEXT NOT NULL CHECK (goal IN ('sales', 'installs', 'awareness', 'traffic')),
  geo TEXT[] NOT NULL,
  audience JSONB NOT NULL,
  budget JSONB NOT NULL,
  model TEXT NOT NULL CHECK (model IN ('cpa', 'cpp', 'cpm', 'fixed')),
  utm JSONB NOT NULL,
  promo_codes TEXT[],
  status TEXT NOT NULL DEFAULT 'draft' CHECK (status IN ('draft', 'pending', 'active', 'paused', 'completed', 'disputed')),
  integrations JSONB DEFAULT '{"ga4": false, "appsflyer": false, "shopify": false}',
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE public.escrows (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  deal_id UUID NOT NULL REFERENCES public.placements(id) ON DELETE CASCADE,
...
CREATE POLICY "Creators can create channels" ON public.channels
  FOR INSERT WITH CHECK (
    auth.uid() = owner_user_id AND
    EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role = 'creator')
  );

CREATE POLICY "Creators can update own channels" ON public.channels
  FOR UPDATE USING (
    auth.uid() = owner_user_id AND
    EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role = 'creator')
  );
...
-- Formats policies
CREATE POLICY "Anyone can view formats" ON public.formats
  FOR SELECT USING (true);

CREATE POLICY "Creators can manage own formats" ON public.formats
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.channels 
      WHERE id = formats.channel_id AND owner_user_id = auth.uid()
    )
  );
...
-- Placements policies
CREATE POLICY "Related users can view placements" ON public.placements
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.campaigns WHERE id = placements.campaign_id AND advertiser_id = auth.uid()
    ) OR
    EXISTS (
      SELECT 1 FROM public.channels WHERE id = placements.channel_id AND owner_user_id = auth.uid()
    ) OR
    EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role = 'admin')
  );
...
CREATE POLICY "Related users can view analytics" ON public.analytics_events
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.placements p
      JOIN public.campaigns c ON c.id = p.campaign_id
      WHERE p.id = analytics_events.placement_id AND c.advertiser_id = auth.uid()
    )
  );

-- Cases policies (public)
