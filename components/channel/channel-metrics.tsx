...
export function ChannelMetrics({ channel }: ChannelMetricsProps) {
  ...
  const metricsData = [
    ...
    },
    {
      icon: Target,
      label: 'Охват',
      value: formatNumber((metrics as any).reach || metrics.avg_views || 0),
    ...
    },
    {
      icon: Zap,
      label: 'Вовлечённость',
      value: formatNumber((metrics as any).engagement || 0),
      color: 'text-pink-600',
      bgColor: 'bg-pink-50',
    },
    {
      icon: DollarSign,
      label: 'Средняя цена',
      value: (metrics as any).avg_price ? `${formatNumber((metrics as any).avg_price)} ₽` : 'По запросу',
